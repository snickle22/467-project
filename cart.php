...
/* Getting shipping cost from database
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'bulk_add') {
        ...
        try {
            ...
            $insertOrder = $new_pdo->prepare("INSERT INTO orders (total_price, shipping_address, shipping_charge) VALUES (?, ?, ?)");
            $insertOrder->execute([$actualTotal, $shipping_address, $shipping_charge]);
            $order_id = $new_pdo->lastInsertId();

            foreach ($_SESSION['cart'] as $item) {
                $check = $new_pdo->prepare("SELECT stock FROM inventory WHERE part_number = ? FOR UPDATE");
                ...
                $updateStock = $new_pdo->prepare("UPDATE inventory SET stock = stock - ? WHERE part_number = ?");
